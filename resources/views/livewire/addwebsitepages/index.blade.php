...
use App\Models\Page;
use App\Livewire\Forms\PagesForm;
...
new class extends Component {
      ...
      public function getDetails(Page $page){
         $this->form->title = $page->pagename;
         $this->form->taxonnomyCode = $page->taxonomycode;
         $this->editId = $page->id;
         $this->mode = 'edit';
      }

      public function deleteDetails($taxonomyId){
         Page::where('id',$taxonomyId)->delete();
      }
        ...
        <div class="mb-4.5">
          ...
          >
                <select
                    ...
                    wire:model="form.taxonnomyCode"
                    >
                    <option value="" class="text-body">
                        Taxonomy
                    </option>
                    @foreach($Taxonomies as $taxonomyKey => $taxonomyValue )
                      <option value="{{$taxonomyValue->id}}" class="text-body" {{$taxonomyValue->id == $this->editId?'selected':'' }} >{{$taxonomyValue->title}}</option>
                    @endforeach
                </select>
                <x-input-error :messages="$errors->get('form.taxonnomyCode')" class="mt-2" />
  ...
  <div class="mt-2 bg-white border rounded-sm border-stroke shadow-default dark:border-strokedark dark:bg-boxdark">
    <div class="max-w-full overflow-x-auto data-table-common data-table-two">
       <div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns">
                   ...
                   <tr data-index="0">
                      ...
                      <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                         <div class="flex items-center space-x-3.5">
                           <button type="button" class="hover:text-primary" wire:click="getDetails({{$page->id}})">
                           ...
                           <button class="hover:text-primary" wire:click="deleteDetails({{$page->id}})">
                              <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                           ...
                           <a href="{{route('addhtmlblock',$page->id)}}" wire:navigate>
                              <svg width="18px" height="18px" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" version="1.1" >
                           ...
                           <a href="{{route('addhtmlsubblock',$page->id)}}" wire:navigate>
                              <svg width="18px" height="18px" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" version="1.1" >
                                 <path style="fill:#333333;stroke:none;" d="m 50,46 -22,-22 22,-22 22,22 -22,22"/>
                                 <path style="fill:#333333;stroke:none;" d="m 48,48 -22,-22 -22,22 22,22 22,-22"/>
                                 <path style="fill:#333333;stroke:none;" d="m 52,48 22,-22 22,22 -22,22 -22,-22"/>
          ...
          <div class="p-5">
              {{$pages->links('vendor.livewire.tailwind')}}
          </div>
       </div>
    </div>
 </div>
