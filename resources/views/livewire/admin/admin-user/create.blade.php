...
<div>
    ...
    <form wire:submit='save'>
        ...
        <x-title  class="mt-4" wire:model="form.email" caption="Email" name="'email'" />
        <x-field-error :messages="$errors->get('form.email')" class="mt-2 required" />

        <x-title  class="mt-4" wire:model="form.password" caption="Password" name="'password'" />
        <x-field-error :messages="$errors->get('form.password')" class="mt-2 required" />

        <x-title  class="mt-4" wire:model="form.confirmPassword" caption="Confirm Password" name="'confirmPassword'" />
        ...
        <div class="mt-4">
            ...
            <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-white dark:bg-form-input">
                <select class="relative z-20 w-full py-3 pl-5 pr-12 transition bg-transparent border rounded outline-none appearance-none border-stroke focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" :class="isOptionSelected &amp;&amp; 'text-black dark:text-white'" @change="isOptionSelected = true" wire:model="form.role">
                    <option value="">
                        Select Role
                    </option>
