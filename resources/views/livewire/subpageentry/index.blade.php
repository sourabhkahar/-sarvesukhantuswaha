...
new class extends Component {
    ...
    public function with(): array{
        ...
        'pages' => Page::Where([
                                    ...
                                    'status'=>'Y'
                                ])->search($this->search)->orderBy($this->sortColumn,$this->sortDirection)->paginate($this->perPage),
        ];
    }

    public function deleteDetails($padeId){
        $page = Page::find($padeId);
        $page->status = 'N';
        $page->save();
    ...
    public function mount($cid,$tid){
        $this->cid = $cid;
        $this->tid = $tid;
        $this->taxonomy = Taxonomy::where('id',$this->tid)->first();
    }

    public function addNewPage(){
        $Pagecode = Page::max('id');
        $this->pid = $Pagecode + 1;
        $this->redirect("/admin/pageentry/$this->pid/$this->tid/$this->cid/add", navigate: true); 
    }
                            ...
                            <tr data-index="0">
                                ...
                                <td class="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <div class="flex items-center space-x-3.5">
                                        <a href="{{route('pageentry',['pid'=>$page->id,'tid'=>$tid,'cid'=>$cid,'act'=>'edit'])}}"
                                            type="button" class="hover:text-primary" wire:navigate>
                                        ...
                                        <button class="hover:text-primary" wire:click="deleteDetails({{$page->id}})" wire:confirm="Are you sure you want to delete this Item?">
                                            <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18"
                                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
